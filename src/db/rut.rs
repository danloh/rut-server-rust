...
use crate::model::rut::{
    CreateRut, QueryRut, QueryRuts, Rut, StarOrRut, StarRut, StarRutStatus, UpdateRut,
};
...
impl Handler<QueryRut> for Dba {
    ...
    fn handle(&mut self, rslug: QueryRut, _: &mut Self::Context) -> Self::Result {
        ...
        let rut_query = ruts
            .filter(&slug.eq(&rslug.rut_slug)) // slug here only
            .get_result::<Rut>(conn)?;
...
impl Handler<QueryRuts> for Dba {
    ...
    fn handle(&mut self, per: QueryRuts, _: &mut Self::Context) -> Self::Result {
        ...
        match per {
            ...
            QueryRuts::KeyID(k, f, i, p) => {
                // per keyword from taged, created, collected
                let fr = f.trim();
                match fr {
                    "user" => {
                        // just use this arm
                    ...
                    "tag" => {
                        ...
                        use crate::schema::tagruts::dsl::{rut_id, tagruts, tname};
                        let ids = tagruts
                            .filter(&tname.eq(&i))
                            .select(rut_id)
                            .load::<String>(conn)?;
...
impl Handler<StarOrRut> for Dba {
    ...
    fn handle(&mut self, rstar: StarOrRut, _: &mut Self::Context) -> Self::Result {
        ...
        use crate::schema::ruts::dsl::{
            comment_count, id as rid, item_count, ruts, star_count, vote,
        };
        let rut_query = ruts
            .filter(&rid.eq(&rstar.rut_id))
            .get_result::<Rut>(conn)?;
...
impl Handler<StarRutStatus> for Dba {
    ...
    fn handle(&mut self, status: StarRutStatus, _: &mut Self::Context) -> Self::Result {
        ...
        use crate::schema::ruts::dsl::{id as rid, ruts, star_count};
        let s_count = ruts
            .filter(&rid.eq(&status.rut_id))
            .select(star_count)
            .get_result::<i32>(conn)?;
